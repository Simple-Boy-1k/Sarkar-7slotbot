...
def normalize_text(text: str) -> str:
    """Fancy unicode font 'ꜱʟᴏᴛ' ko standard 'slot' mein convert karta hai"""
    if not text:
        return ""
    # Fancy small-caps replacement
    text = text.replace("ꜱ", "s").replace("ʟ", "l").replace("ᴏ", "o").replace("ᴛ", "t").replace("𝗼", "o")
...
def setup_admin_handlers(app: Client, owner_id: int, send_start_panel_fn):
    ...
    async def handle_admin_inputs(client, message: Message):
        ...
        lower_text = normalize_text(raw_text)

        # =========================================================================
        # 1. PEHLE CHECK KARO KI KYA USER KISI STATE MEIN HAI (Input bhej raha hai)
        # =========================================================================
        ...
        if state:
            if state == "SET_PROMO":
                ...
                return await message.reply_text("✅ Promo Text Updated Successfully!")

            elif state == "SET_GET_KEY":
                set_setting("get_key_url", raw_text)
                user_states.pop(user_id, None)
                return await message.reply_text("✅ Get Key Link Saved Successfully!")

            elif state == "SET_CLICK_HERE":
                set_setting("click_url", raw_text)
                user_states.pop(user_id, None)
                return await message.reply_text("✅ Click Here Link Saved Successfully!")

            elif state == "SET_VERIFY":
                set_setting("verify_url", raw_text)
                user_states.pop(user_id, None)
                return await message.reply_text("✅ Verify Link Saved Successfully!")

            elif state == "SET_OFFLINE_CHAN":
                set_setting("offline_channel", raw_text)
                user_states.pop(user_id, None)
                return await message.reply_text("✅ Offline Channel Saved Successfully!")
                ...
                user_states.pop(user_id, None)
                return await status_msg.edit_text(f"✅ **Broadcast Completed!**\n\n📤 Sent: `{sent}`\n❌ Failed: `{failed}`")
                ...
                if message.photo:
                    ...
                    user_states.pop(user_id, None)
                    return await message.reply_text("✅ DP Updated (Photo)!")
                elif message.video:
                    set_setting("media_file_id", message.video.file_id)
                    set_setting("media_type", "video")
                    user_states.pop(user_id, None)
                    return await message.reply_text("✅ DP Updated (Video)!")
                else:
                    return await message.reply_text("❌ Please send a valid Photo or Video.")

            elif state == "SET_VOICE":
                if message.voice:
                    set_setting("voice_file_id", message.voice.file_id)
                    user_states.pop(user_id, None)
                    return await message.reply_text("✅ Voice Updated!")
                else:
                    return await message.reply_text("❌ Please send a valid Voice message.")
                ...
                user_states.pop(user_id, None)
                return await message.reply_text(f"✅ SLOT {slot_id} Saved Successfully!\n\n🔗 Link: {link}")

        # =========================================================================
        # 2. AGAR KOI STATE NAHI HAI, TOH BUTTON CLICKS CHECK KARO
        # =========================================================================
        
        # Check Slot Clicks (Slot 1 to 7) & Remove Slot Clicks
        for n in range(1, 8):
            if f"slot {n}" in lower_text or f"slot{n}" in lower_text:
                if "remove" in lower_text or "❌" in raw_text:
